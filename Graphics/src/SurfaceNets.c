...
void init_surfacenets(void)
{
	...
	for(i = 0; i < 256; ++i)
	{
		...
		for(j = 0; j < 24; j += 2)
		{
			int a = !(
				i & (1 << cube_edges
						 [j])); // was !!, which in js turns into boolean false null, undefined, etc
			...
			for(x[0] = 0; x[0] < dims[0] - 1; ++x[0], ++n, ++m)
			{
				...
				for(i = 0; i < 3; ++i)
				{
					...
					if(storeFlag)
					{
						...
						{
							faces[faces_length++] =
								(int3D){buffer[m], buffer[m - du - dv], buffer[m - du]};
							faces[faces_length++] =
								(int3D){buffer[m], buffer[m - dv], buffer[m - du - dv]};
						}
						else
						{
							faces[faces_length++] =
								(int3D){buffer[m], buffer[m - du - dv], buffer[m - dv]};
							faces[faces_length++] =
								(int3D){buffer[m], buffer[m - du], buffer[m - du - dv]};
						}
					}
					else
						faces_length += 2;
...
int main(int argc, char* argv[])
{
	...
	for(i = 0; i < dim[0]; i++)
		for(j = 0; j < dim[1]; j++)
			for(k = 0; k < dim[2]; k++)
				if(pow(i - dim[0] / 2, 2) + pow(j - dim[1] / 2, 2) + pow(k - dim[2] / 2, 2) <
				   10 * 10)
